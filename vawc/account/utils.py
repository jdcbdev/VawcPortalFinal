import os
from django.conf import settings
from account.models import Email, Recaptcha, Twilio

# Set email settings config
def load_settings():
    if os.environ.get('DJANGO_EMAIL_HOST_USER') and os.environ.get('DJANGO_EMAIL_HOST_PASSWORD'):
        settings.DEFAULT_FROM_EMAIL = os.environ.get('DJANGO_EMAIL_HOST_USER')
        settings.EMAIL_HOST = os.environ.get('DJANGO_EMAIL_HOST', 'smtp.gmail.com')
        settings.EMAIL_PORT = int(os.environ.get('DJANGO_EMAIL_PORT', 587))
        settings.EMAIL_HOST_USER = os.environ.get('DJANGO_EMAIL_HOST_USER')
        settings.EMAIL_HOST_PASSWORD = os.environ.get('DJANGO_EMAIL_HOST_PASSWORD')
    else:
        email = Email.objects.all().first()
        if email:
            settings.DEFAULT_FROM_EMAIL = email.email
            settings.EMAIL_HOST = email.host
            settings.EMAIL_PORT = email.port
            settings.EMAIL_HOST_USER = email.email
            settings.EMAIL_HOST_PASSWORD = email.password
        else:
            raise ValueError("No email settings found in environment or database.")

def load_recaptcha_settings():
    if os.environ.get('RECAPTCHA_SECRET_KEY') and os.environ.get('RECAPTCHA_SITE_KEY'):
        settings.RECAPTCHA_SECRET_KEY = os.environ.get('RECAPTCHA_SECRET_KEY')
        return {
            'site_key': os.environ.get('RECAPTCHA_SITE_KEY'),
            'secret_key': os.environ.get('RECAPTCHA_SECRET_KEY')
        }
    else:
        recaptcha = Recaptcha.objects.all().first()
        if recaptcha:
            settings.RECAPTCHA_SECRET_KEY = recaptcha.secret_key
            return {'site_key': recaptcha.site_key, 'secret_key': recaptcha.secret_key}
        else:
            raise ValueError("No reCAPTCHA settings found in environment or database.")


def load_twilio_settings():
    if os.environ.get('TWILIO_ACCOUNT_SID') and os.environ.get('TWILIO_AUTH_TOKEN'):
        settings.TWILIO_ACCOUNT_SID = os.environ.get('TWILIO_ACCOUNT_SID')
        settings.TWILIO_AUTH_TOKEN = os.environ.get('TWILIO_AUTH_TOKEN')
        settings.TWILIO_PHONE_NUMBER = os.environ.get('TWILIO_PHONE_NUMBER')
        settings.TWILIO_TYPE = os.environ.get('TWILIO_TYPE', 'local')
        return {'twilio_type': settings.TWILIO_TYPE}
    else:
        twilio = Twilio.objects.all().first()
        if twilio:
            settings.TWILIO_ACCOUNT_SID = twilio.account_sid
            settings.TWILIO_AUTH_TOKEN = twilio.auth_token
            settings.TWILIO_PHONE_NUMBER = twilio.phone_number
            settings.TWILIO_TYPE = twilio.type
            return {'twilio_type': twilio.type}
        else:
            raise ValueError("No Twilio settings found in environment or database.")